...
if [ "$1" = 'rabbitmq-server' ] && [ "$haveConfig" ]; then
  # Reset access to working directories
  mkdir -p ${RABBITMQ_HOME} ${RABBITMQ_HOME} ${RABBITMQ_CONFIG_BASE} ${RABBITMQ_LOG_BASE}
  chown -R ${RABBITMQ_USER}: ${RABBITMQ_HOME} ${RABBITMQ_CONFIG_BASE} ${RABBITMQ_LOG_BASE}
  ...
  rabbitConfig=(
    ...
    "{ log_levels, [{autocluster, info}, {connection, info}, {channel, warning}, {federation, info}, {mirroring, info}, {shovel, info}] }"
    "{ vm_memory_high_watermark, 0.95 }"
    "{ vm_memory_high_watermark_paging_ratio, 0.7 }"
    "{ tcp_listen_options, [{backlog, 128}, {nodelay, true}, {linger, {true, 0}}, {exit_on_close, false}, {sndbuf, 196608}, {recbuf, 196608}] }"
  )
  ...
  echo "$(rabbit_array "${fullConfig[@]}")." > "${RABBITMQ_CONFIG_BASE}/rabbitmq.config"
fi
